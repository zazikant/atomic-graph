...
export function NodeDrawer() {
  ...
  return (
    <div className="absolute right-0 top-0 bottom-0 w-[320px] bg-[#12122a] border-l border-[#2a2a5a] z-50 flex flex-col animate-in slide-in-from-right duration-200">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#2a2a5a]">
        <div className="flex items-center gap-2">
          <div
            className="w-2 h-2 rounded-full"
            style={{ backgroundColor: color }}
          />
          <h3 className="text-[#e0e0ff] font-mono text-sm font-bold truncate max-w-[200px]">
            {data.title}
          </h3>
        </div>
        <Button
              ...
              <div className="space-y-1.5">
                {connectedNodes.map((cn) => {
                  ...
                  return (
                    <div
                      key={cn.id}
                      className="flex items-center gap-2 px-2 py-1.5 rounded bg-[#1a1a3e] border border-[#2a2a5a] cursor-pointer hover:border-[#4a4a8a] transition-colors"
                      onClick={() => setSelectedNodeId(cn.id)}
                    >
                      <Link2 className="w-3 h-3 text-[#6366f1] shrink-0" />
                      <span className="text-[#c8c8ee] font-mono text-xs truncate flex-1">
                        {cn.data.node.title}
                      </span>
                      {edge?.label && (
                        <span className="text-[#6666aa] font-mono text-[10px] truncate max-w-[80px]">
                          {edge.label}
                        </span>
                      )}
                      <ExternalLink className="w-2.5 h-2.5 text-[#5555aa]" />
                    </div>
                  );
                })}
              </div>
